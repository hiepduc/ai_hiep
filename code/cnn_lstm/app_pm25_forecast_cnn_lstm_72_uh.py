...
def get_forecast_from_api(start_date_str, end_date_str):
    ...
    target_sites = {
        "MERRIWA": site_map["MERRIWA"],
        "MUSWELLBROOK": site_map["MUSWELLBROOK"],
        "SINGLETON": site_map["SINGLETON"]
    }
...
if st.button("Run Forecast"):
    try:
        with st.spinner("Fetching data and running forecast..."):
            ...
            model = load_model("cnn_lstm_model_pm25_uh_72input.h5")

            forecast_hours = forecast_days * 24
            scaler = joblib.load("pm25_uh_scaler.save")
            feature_columns = joblib.load("pm25_uh_columns.save")
