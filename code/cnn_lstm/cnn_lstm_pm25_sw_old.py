...
N_INPUT = 72     # Use 72 hours as input (3 days)
...
n_steps_per_subseq = N_INPUT // N_SUBSEQ  # 72 / 6 = 12
...
model.save("cnn_lstm_model_pm25_72input.h5")

# === STEP 8: PREDICT & INVERSE SCALE ===
y_pred = model.predict(X_val)

# Flatten sequences to compare true vs predicted
...
print(f"\nMean Absolute Error (µg/m³): {mae:.2f}")

# === STEP 10: PLOT COMPARISON (example: Liverpool station) ===
site_index = df.columns.get_loc('PM2.5_LIVERPOOL')

plt.figure(figsize=(10, 5))
plt.plot(y_val_inv[:, site_index], label='Actual')
plt.plot(y_pred_inv[:, site_index], label='Predicted')
plt.title('PM2.5 Forecast at LIVERPOOL')
...
site_index = df.columns.get_loc('PM2.5_BARGO')
plt.figure(figsize=(10, 5))
plt.plot(y_val_inv[:, site_index], label='Actual')
plt.plot(y_pred_inv[:, site_index], label='Predicted')
plt.title('PM2.5 Forecast at BARGO')
plt.xlabel('Time Step (hour)')
plt.ylabel('PM2.5 (µg/m³)')
plt.legend()
plt.tight_layout()
...
site_index = df.columns.get_loc('PM2.5_BRINGELLY')
plt.figure(figsize=(10, 5))
plt.plot(y_val_inv[:, site_index], label='Actual')
plt.plot(y_pred_inv[:, site_index], label='Predicted')
plt.title('PM2.5 Forecast at BRINGELLY')
plt.xlabel('Time Step (hour)')
plt.ylabel('PM2.5 (µg/m³)')
plt.legend()
plt.tight_layout()
...
site_index = df.columns.get_loc('PM2.5_CAMPBELLTOWN_WEST')
plt.figure(figsize=(10, 5))
plt.plot(y_val_inv[:, site_index], label='Actual')
plt.plot(y_pred_inv[:, site_index], label='Predicted')
plt.title('PM2.5 Forecast at CAMPBELLTOWN_WEST')
plt.xlabel('Time Step (hour)')
plt.ylabel('PM2.5 (µg/m³)')
plt.legend()
plt.tight_layout()
