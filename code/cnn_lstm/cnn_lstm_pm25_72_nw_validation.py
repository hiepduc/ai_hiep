...
joblib.dump(scaler, "pm25_nw_scaler.save")
joblib.dump(df.columns.tolist(), "pm25_nw_columns.save")
...
history = model.fit(X_train, y_train,
                    ...
                    verbose=2)
model.save("cnn_lstm_model_pm25_nw_72input.h5")
...
summary_df.to_csv("test_summary_metrics_nw.csv", index=False)
...
pd.DataFrame(site_mae_list).to_csv("test_site_mae_nw.csv", index=False)

# === STEP 10: Save actual vs predicted
test_index = pd.date_range(start='2025-01-01', periods=len(y_test_inv), freq='H')  # adjust start as needed
...
df_combined.to_csv("test_predictions_vs_actual_nw.csv")

# === STEP 11: Interactive Plot
sites_to_plot = site_names  # All three NW sites
...
fig.update_layout(height=300*len(sites_to_plot),
                  title_text="PM2.5 Forecast vs Actual on Test Set (NW Sydney)", showlegend=True)
fig.write_html("test_forecast_dashboard_nw.html")
print("✅ Interactive dashboard saved to test_forecast_dashboard_nw.html")
