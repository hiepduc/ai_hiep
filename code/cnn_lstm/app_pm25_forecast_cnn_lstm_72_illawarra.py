...
def get_forecast_from_api(start_date_str, end_date_str):
    ...
    target_sites = {
        "ALBION_PARK_SOUTH": site_map["ALBION PARK SOUTH"],
        "KEMBLA_GRANGE": site_map["KEMBLA GRANGE"],
        "WOLLONGONG": site_map["WOLLONGONG"]
    }
...
if st.button("Run Forecast"):
    try:
        with st.spinner("Fetching data and running forecast..."):
            ...
            model = load_model("cnn_lstm_model_pm25_illawarra_72input.h5")

            forecast_hours = forecast_days * 24
            scaler = joblib.load("pm25_illawarra_scaler_72.save")
            feature_columns = joblib.load("pm25_illawarra_columns_72.save")
