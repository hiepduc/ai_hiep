...
df_predictions = run_forecast(start_date_str, end_date_str, scaler, feature_columns)

df_recent = fetch_recent_data(start_date_str, end_date_str)

def plot_predictions(df_predictions, df_input, start_date_str, end_date_str):
    """
    Plots the time series of predicted PM2.5 along with the input features (WSP, Temp, RH) from Liverpool.
    """
    ...
    # Plot PM2.5 predictions for each site
    ...
    axes[0].set_title('Predicted PM2.5 at Liverpool')
    axes[0].set_ylabel('PM2.5 (µg/m³)')
    axes[0].legend(loc='upper left')

    # Plot PM2.5 at other sites
    ...
    axes[1].set_title('Predicted PM2.5 at Bringelly')
    ...
    axes[2].set_title('Predicted PM2.5 at Campbelltown West')
    ...
    axes[3].set_title('Predicted PM2.5 at Camden')
    ...
    axes[4].plot(time_range_input, df_input["LIV_Temp"][:len(time_range_input)], label="Temperature (TEMP)", color='tab:cyan', linestyle='-', linewidth=2)
    axes[4].plot(time_range_input, df_input["LIV_RH"][:len(time_range_input)], label="Relative Humidity (RH)", color='tab:pink', linestyle='-.', linewidth=2)
...
# Example usage:
plot_predictions(df_predictions, df_recent, start_date_str, end_date_str)
