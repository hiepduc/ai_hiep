...
#########
            if "VMD" in model_type:
                ...
                for site_base_name in site_base_names:
                    ...
                    historical_recon = None
                    if all(col in df_obs.columns for col in imf_cols):
                        historical_recon = df_obs[imf_cols].sum(axis=1).rename(recon_col_name)
                    ...
                    if all(col in forecast_df.columns for col in imf_cols):
                        ...
                        forecast_df[recon_col_name] = forecast_recon  # Save it

                    # Skip if neither available
                    if historical_recon is None and forecast_recon is None:
                        st.warning(f"⚠️ Missing IMF columns for {site_base_name}, skipping reconstruction.")
                        continue

                    # --- Combine Historical + Forecast ---
                    combined_recon = pd.concat([historical_recon, forecast_recon])

                    # --- Actual PM2.5 from API ---
                    actual_pm25 = actual_future[[site_base_name]].dropna() if site_base_name in actual_future.columns else None

                    # --- Plot ---
                    df_plot = pd.DataFrame({
                        "Reconstructed PM2.5": combined_recon,
                    })
                    if actual_pm25 is not None and not actual_pm25.empty:
                        df_plot["Observed PM2.5"] = actual_pm25.reindex(df_plot.index)

                    df_plot = df_plot.dropna(how="all").reset_index().rename(columns={"index": "datetime"})
                    ...
                    fig = px.line(df_melted, x="datetime", y="PM2.5", color="Type",
                                  title=f"🌀 Historical + Forecast Reconstructed vs Observed: {site_base_name}")
                    st.plotly_chart(fig, use_container_width=True, key=f"recon_{site_base_name}")

                    # Optional metrics on forecast part only
                    if forecast_recon is not None and actual_pm25 is not None:
                        forecast_range = forecast_recon.index
                        actual_trimmed = actual_pm25.reindex(forecast_range).dropna()
