...
# Rolling Forecast
# -------------------------------
def run_forecast(df_recent, model, scaler, forecast_hours, feature_columns, n_input):
    if n_input == 72:
        n_subseq = 6
    elif n_input == 120:
        n_subseq = 10
    ...
    df_input = df_recent[-n_input:][feature_columns]
...
if st.button("Run Forecast"):
    with st.spinner("Fetching data and running model..."):
        ...
        if len(df) < n_input:
            st.error(f"❌ Not enough past data (need at least {n_input} hours).")
            st.stop()

        model_path = f"models/cnn_lstm_model_pm25_{region.lower()}_{n_input}input.h5"
        scaler_path = f"scalers/pm25_{region.lower()}_scaler_{n_input}.save"
        columns_path = f"scalers/pm25_{region.lower()}_columns_{n_input}.save"

        model = load_model(model_path)
        scaler = joblib.load(scaler_path)
        feature_columns = joblib.load(columns_path)

        forecast_df = run_forecast(df, model, scaler, forecast_hours, feature_columns, n_input)
    ...
    # Plot and CSV
    # -------------------------------
    observed_df = df.tail(n_input).copy()
