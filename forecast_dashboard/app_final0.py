...
def run_forecast(df, model, scaler, feature_columns, n_input, forecast_hours):
    if n_input == 72:
        ...
        raise ValueError("Unsupported input window")
...
st.title("🌫️ Multi-Region PM2.5 Forecast Dashboard ")

main_col, map_col = st.columns([4, 1])
...
with main_col:
    with st.sidebar:
        ...
        model_type = st.radio("Model Type", ["CNN_LSTM", "CNN_LSTM_MET", "CNN_LSTM_TRANSFORMER"])
    ...
    if st.button("Run Forecast"):
        for region in regions_selected:
            ...
            try:
                ...
                elif model_type == "CNN_LSTM_MET":
                    st.info(f"Model type '{model_type}' selected. Placeholder — functionality not yet implemented.")
                    continue
                elif model_type == "CNN_LSTM_TRANSFORMER":
                    st.info(f"Model type '{model_type}' selected. Placeholder — functionality not yet implemented.")
                    continue
                else:
                    st.warning("Unknown model type selected.")
                    continue
            except Exception as e:
