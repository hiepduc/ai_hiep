            ...
            if "VMD" in model_type:
                ...
                for site_base_name in site_base_names:
                    ...
                    if all(col in forecast_df.columns for col in imf_cols):
                        forecast_recon = forecast_df[imf_cols].sum(axis=1)
                    ...
                    if site_base_name not in df_obs_raw.columns:
                        st.warning(f"⚠️ Observation missing for {site_base_name}, skipping.")
                        continue

                    # --- Separate Observed Historical and Observed Future ---
                    df_obs_full = df_obs_raw[[site_base_name]].dropna()
                    df_obs_full.columns = ["Observed"]

                    end_date = pd.to_datetime(end_date)

                    from datetime import timedelta

                    # Split historical observations up to end_date
                    observed_historical = df_obs_full[df_obs_full.index <= end_date]

                    # --- Fetch observed PM2.5 after end_date from API ---
                    future_start = (end_date + pd.Timedelta(hours=1)).strftime('%Y-%m-%dT%H:%M:%S')
                    forecast_horizon_hours = len(forecast_df)  # assuming forecast_df has hourly forecasts
                    future_end = (end_date + pd.Timedelta(hours=forecast_horizon_hours)).strftime('%Y-%m-%dT%H:%M:%S')

                    try:
                        df_future_obs_api = fetch_pm25_data(future_start, future_end, region)
                        df_future_obs_api.columns = df_future_obs_api.columns.str.upper()  # standardize
    
                        site_column_upper = site_base_name.upper()
                        if site_column_upper in df_future_obs_api.columns:
                            actual_future = df_future_obs_api[[site_column_upper]].rename(columns={site_column_upper: "Future Observed"})
                        else:
                            st.warning(f"⚠️ No future observed data for {site_base_name} from API.")
                            actual_future = pd.DataFrame()
                    except Exception as e:
                        st.warning(f"⚠️ Could not fetch future observed data from API: {e}")
                        actual_future = pd.DataFrame()

                    # --- Combine All ---
                    recon_pred = forecast_df[[recon_col_name]].rename(columns={recon_col_name: "Reconstructed"})

                    df_plot = pd.concat([
                        observed_historical.rename(columns={site_base_name: "Observed"}),
                        recon_pred,
                        actual_future  # already renamed to "Future Observed"
                    ], axis=1)
                    ...
                    fig = px.line(df_melted, x="datetime", y="PM2.5", color="Type",
                                  title=f"🟢 Observed vs Predicted PM2.5: {site_base_name}",
                                  color_discrete_map={
                                      "Observed": "black",
                                      "Reconstructed": "blue",
                                      "Future Observed": "red"
                                  })
                    st.plotly_chart(fig, use_container_width=True, key=f"obs_pred_plot_{site_base_name}")

                    # --- Forecast Metrics ---

                    if not actual_future.empty and "Future Observed" in actual_future.columns:
                        # Align reconstructed and actual future PM2.5
                        # Align reconstructed and actual future PM2.5 by index
                        combined_df = recon_pred.join(actual_future, how="inner").rename(columns={
                            "Reconstructed": "reconstructed",
                            "Future Observed": "actual"
                        }).dropna()

                        # Extract cleaned arrays
                        recon_trimmed = combined_df["reconstructed"].values
                        actual_trimmed = combined_df["actual"].values
...
#############
                ...
                st.write("Columns in actual_future before VMD:", actual_future.columns.tolist())
                # 🔁 Apply VMD to actual_future as well
                actual_future = apply_vmd_to_df(actual_future)
                st.write("Columns in actual_future after VMD:", actual_future.columns.tolist())
